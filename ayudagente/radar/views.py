...
def event_graph(request, event_id: int):
    ...
    open_requirements = Prefetch(
        ...
        ).select_related('resource', 'destination', 'location'),
    ...
    for actor in actors:
        ...
        if location is None and actor.open_requirements:
            location = actor.open_requirements[0].location
        nodes.append({
            'id': actor.id,
            ...
            'requirements': [
                _requirement(req) for req in actor.open_requirements
            ],
        })
    ...
    matches = (
        ...
        .select_related(
            'need__actor', 'need__resource',
            'offer__actor', 'via_transport__actor',
        )
    )
    edges = [
        {
            ...
            'via_transport_actor': (
                match.via_transport.actor_id if match.via_transport else None
            ),
