...
class EventListTests(TestCase):
    def test_lists_active_events_only(self):
        active = make_event(name='Sismo activo')
...
class EventGraphTests(TestCase):
    ...
    def test_graph_hides_merged_actors_covered_requirements_discarded_matches(self):
        ...
        need = make_requirement(
            self.event, actor, self.agua, make_location(PEREIRA, 'y'),
        )
        offer = make_requirement(
            self.event, make_actor(self.event, 'Oferente'), self.agua,
            make_location(DOSQUEBRADAS, 'z'), direction=Direction.OFFERS,
        )
