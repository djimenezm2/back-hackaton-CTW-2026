...
class FindRequirementsTests(TestCase):
    ...
    def test_orders_by_real_proximity_and_walks_the_tree(self):
        ...
        near = make_requirement(
            self.event, near_actor, self.alimentos,
            make_location(DOSQUEBRADAS, 'Centro Dosquebradas'),
        )
        far = make_requirement(
            self.event, far_actor, self.alimentos,
        ...
        results = list(
            find_requirements(
                self.event.id, Direction.NEEDS, resource=self.mascotas, near=PEREIRA
            )
        )
    ...
    def test_saturated_requirements_are_excluded(self):
        ...
        make_requirement(
            ...
            quantity=Decimal(20), covered_quantity=Decimal(20),
        )
...
class MatchingPassTests(TestCase):
    ...
    def test_allocation_prefers_near_offer_and_flags_unreachable(self):
        ...
        need = make_requirement(
            self.event, need_actor, self.agua,
            make_location(PEREIRA, 'Barrio Cuba'),
            quantity=Decimal(100),
        )
        offer_near = make_requirement(
            self.event, make_actor(self.event, 'Vecino Dosquebradas'), self.agua,
            make_location(DOSQUEBRADAS, 'Dosquebradas'),
            direction=Direction.OFFERS, quantity=Decimal(100),
        )
        # Far offer with no transport: not a candidate
        make_requirement(
            self.event, make_actor(self.event, 'Donante Cali'), self.agua,
            make_location(CALI, 'Cali'),
            direction=Direction.OFFERS, quantity=Decimal(500),
        )
        # A need nobody can serve (no compatible offer)
        arena = make_resource('arena')
        unreachable = make_requirement(
            self.event, make_actor(self.event, 'Sin suerte'), arena,
    ...
    def test_far_pair_matches_only_through_transport(self):
        need = make_requirement(
            self.event, make_actor(self.event, 'Quibdó centro'), self.agua,
            make_location(QUIBDO, 'Quibdó centro'), quantity=Decimal(500),
        )
        offer = make_requirement(
            self.event, make_actor(self.event, 'Bodega Cali'), self.agua,
        ...
        make_requirement(
            self.event, make_actor(self.event, 'Transportador'), self.transporte,
            make_location(CALI, 'Cali salida'),
            direction=Direction.OFFERS, quantity=Decimal(1),
        )
    ...
    def test_frozen_matches_survive_recomputation(self):
        need = make_requirement(
            self.event, make_actor(self.event, 'Barrio'), self.agua,
            make_location(PEREIRA, 'Barrio'), quantity=Decimal(10),
        )
        offer = make_requirement(
            self.event, make_actor(self.event, 'Vecino'), self.agua,
            make_location(DOSQUEBRADAS, 'Vecino'),
            direction=Direction.OFFERS, quantity=Decimal(10),
        )
    ...
    def test_propose_match_validates_directions(self):
        need = make_requirement(
            self.event, make_actor(self.event, 'A'), self.agua,
            make_location(PEREIRA, 'A'),
        )
        other_need = make_requirement(
            self.event, make_actor(self.event, 'B'), self.agua,
...
class OutreachTests(TestCase):
    def setUp(self):
        ...
        self.need = make_requirement(
            self.event, make_actor(self.event, 'Barrio'), agua,
            make_location(PEREIRA, 'Barrio'),
        )
        self.offer = make_requirement(
            self.event, make_actor(
                self.event, 'ONG Pereira', kind=ActorKind.NONPROFIT
            ), agua,
