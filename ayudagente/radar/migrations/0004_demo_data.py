"""
Demo dataset for exercising the services and the graph API.

Everything hangs off one Event ("Sismo demo Eje Cafetero") so the reverse migration tears
it down cleanly. The evidence invariant holds even for seeds: every requirement is backed
by an Observation created through a manual HarvestJob.

Geography (kept consistent with the scenario):
- IMPACT zone (reports needs): Pereira, Dosquebradas, Quibdó, Cali and Manizales with 20
  barrios each, plus the towns of Risaralda (Santa Rosa, La Virginia, Marsella), Quindío
  (Armenia, Salento, Filandia) and Valle (Cartago, Tuluá, Buga, Palmira, Yumbo).
- SUPPORT zone (the rest of Colombia, offers help): Bogotá, Medellín, Bucaramanga,
  Barranquilla, Ibagué, Neiva.
- 1 in 4 rows flips direction so both exist everywhere.

Deliberate scenario coverage (curated rows):
- dog-food offer that must match a generic food need via the resource tree
- two transport offers: fixed destination and destination-free (solver decides)
- one need with no compatible offer at all — plantas_electricas stays out of the bulk
  rotation on purpose so this alert always fires
- one saturated requirement (covered == quantity, never proposed again)

Coordinates: city centers are real; barrio points are a deterministic golden-angle spiral
around them (0.5–4 km) — believable urban scatter without hand-geocoding 200 barrios.
"""
...
ACTORS = [
    ...
    ('coliseo_quibdo', 'Coliseo Mayor de Quibdó (La Yesquita)', 'collection_center', 'quibdo_coliseo', '27001', 0.85),
    ...
    ('cruz_roja', 'Cruz Roja Risaralda (Circunvalar)', 'nonprofit', 'cruz_roja_sede', '66001', 0.95),
    ...
    ('vol_manizales', 'Voluntarios Manizales (Chipre)', 'volunteer_group', 'mzl_chipre', '17001', 0.65),
    ('andres', 'Andrés (camioneta, El Poblado)', 'person', 'el_poblado_pei', '66001', 0.55),
    ('transcafe', 'Transportes del Café SAS (Terminal)', 'company', 'terminal_pereira', '66001', 0.85),
    ('dono_mascotas', 'Vecina Santa Mónica', 'person', 'dq_santa_monica', '66170', 0.5),
]

CONTACTS = [
    ('cruz_roja', 'email', 'contacto@cruzrojarisaralda.org', 'contacto@cruzrojarisaralda.org', 0.92),
...
REQUIREMENTS = [
    ...
    ('req_agua_quibdo', 'coliseo_quibdo', 'needs', 'agua', 5000, 0, 'litros', 'critical',
     'quibdo_coliseo', '27001', None, None,
     'Necesitamos agua potable urgente en el Coliseo Mayor, llegaron 400 familias'),
    ('req_meds_hospital', 'hospital_sj', 'needs', 'medicamentos', 200, 0, 'kits', 'critical',
     'hospital_sj', '66001', None, None,
     'Hospital San Jorge requiere kits de primeros auxilios e insulina'),
    ('req_carpas_vereda', 'vereda_florida', 'needs', 'carpas', 50, 0, 'unidades', 'high',
     'vereda_florida', '66001', None, None,
     'En la vereda La Florida 50 familias durmiendo a la intemperie'),
    ('req_alimentos_cuba', 'jac_cuba', 'needs', 'alimentos', 300, 0, 'kg', 'high',
     'barrio_cuba', '66001', None, None,
     'Barrio Cuba necesita mercados, olla comunitaria para 200 personas'),
    ('req_voluntarios_cuba', 'jac_cuba', 'needs', 'voluntarios', 20, 18, 'personas', 'medium',
     'barrio_cuba', '66001', None, None,
     'Faltan manos para remover escombros en Cuba'),
    ('req_colchonetas_coliseo', 'coliseo_quibdo', 'needs', 'colchonetas', 30, 30, 'unidades', 'high',
     'quibdo_coliseo', '27001', None, None,
     'Colchonetas para el coliseo (YA CUBIERTO según última publicación)'),
    ('req_plantas_quibdo', 'coliseo_quibdo', 'needs', 'plantas_electricas', 2, 0, 'unidades', 'high',
     'quibdo_coliseo', '27001', None, None,
     'Sin luz en el coliseo, se necesitan plantas eléctricas'),
    ('off_agua_cali', 'bodega_cali', 'offers', 'agua', 12000, 0, 'litros', 'medium',
     'cali_base', '76001', None, None,
     'Tenemos 12000 litros de agua embotellada listos para despachar desde Cali'),
    ('off_alimentos_utp', 'acopio_utp', 'offers', 'alimentos', 800, 0, 'kg', 'medium',
     'utp', '66001', None, None,
     'Acopio UTP con 800kg de mercados clasificados'),
    ('off_mascotas_dq', 'dono_mascotas', 'offers', 'alimentos_mascotas', 50, 0, 'kg', 'low',
     'dq_santa_monica', '66170', None, None,
     'Tengo 50kg de comida para perros, ¿dónde la llevo?'),
    ('off_cobijas_cr', 'cruz_roja', 'offers', 'colchonetas', 200, 0, 'unidades', 'medium',
     'cruz_roja_sede', '66001', None, None,
     'Cruz Roja dispone de 200 colchonetas y cobijas'),
    ('off_voluntarios_mzl', 'vol_manizales', 'offers', 'voluntarios', 15, 0, 'personas', 'medium',
     'mzl_chipre', '17001', None, None,
     'Grupo de 15 voluntarios disponibles fin de semana'),
    ('off_transcafe', 'transcafe', 'offers', 'transporte', 3, 0, 'vehículos', 'high',
     'terminal_pereira', '66001', 'quibdo_coliseo', '27001',
     'Tres camiones de 5t Pereira-Quibdó, salida diaria'),
    ('off_andres', 'andres', 'offers', 'transporte', 1, 0, 'vehículos', 'medium',
     'el_poblado_pei', '66001', None, None,
...
ZONES = {
    ...
    '66001': ('pereira', 'needs', [
        'Centro', 'Cuba', 'Álamos', 'El Poblado', 'Villa Santana', 'Kennedy',
        'El Jardín', 'Providencia', 'Boston', 'San Nicolás', 'Corocito', 'El Vergel',
        'Los Rosales', 'Pinares', 'Circunvalar', 'Ciudad Jardín', 'Nacederos',
        'Belmonte', 'San Joaquín', 'La Aurora',
    ]),
    '66170': ('dosquebradas', 'needs', [
        'La Popa', 'Santa Mónica', 'Frailes', 'La Badea', 'Santa Teresita',
        'Guadalupe', 'El Japón', 'Los Molinos', 'La Pradera', 'Bosques de la Acuarela',
        'Santiago Londoño', 'La Capilla', 'Buenos Aires', 'Playa Rica', 'La Romelia',
        'Villa Fanny', 'Camilo Torres', 'Aguazul', 'La Esperanza', 'El Balso',
    ]),
    '27001': ('quibdo', 'needs', [
        'La Yesquita', 'Yescagrande', 'Kennedy', 'Niño Jesús', 'Roma', 'Alameda Reyes',
        'Tomás Pérez', 'San Vicente', 'Cristo Rey', 'Las Mercedes', 'El Caraño',
        'Medrano', 'Buenos Aires', 'Samper', 'Pablo Sexto', 'La Aurora', 'Palenque',
        'San Judas', 'Monserrate', 'Las Margaritas',
    ]),
    '76001': ('cali', 'needs', [
        'San Fernando', 'Granada', 'San Antonio', 'El Peñón', 'Versalles', 'Alameda',
        'Tequendama', 'Ciudad Jardín', 'El Ingenio', 'Meléndez', 'La Flora',
        'Chipichape', 'Menga', 'Salomia', 'Junín', 'El Refugio', 'Nápoles', 'La Base',
        'Santa Mónica Norte', 'San Bosco',
    ]),
    '17001': ('manizales', 'needs', [
        'Chipre', 'Palermo', 'El Cable', 'Milán', 'La Enea', 'Fátima', 'Versalles',
        'Campohermoso', 'San Jorge', 'La Sultana', 'Betania', 'El Carmen', 'Solferino',
        'Villa Pilar', 'La Francia', 'Malhabar', 'La Estrella', 'Aranjuez',
        'Lusitania', 'El Bosque',
    ]),
    # --- IMPACT: towns, 6 points each
    '66682': ('santa_rosa', 'needs', [
        'Centro', 'Las Araucarias', 'La Hermosa', 'Cedralito', 'Monserrate', 'La Capilla',
    ]),
    '66400': ('la_virginia', 'needs', [
        'Centro', 'Balsillas', 'El Progreso', 'Pedro Pablo Bello', 'San Fernando', 'La Playa',
    ]),
    '66440': ('marsella', 'needs', [
        'Centro', 'El Rayo', 'La Miranda', 'Beltrán', 'Estación Pereira', 'Alto Cauca',
    ]),
    '63001': ('armenia', 'needs', [
        'Centro', 'La Castellana', 'Granada', 'El Recreo', 'Ciudad Dorada', 'La Fachada',
    ]),
    '63690': ('salento', 'needs', [
        'Centro', 'Boquía', 'Cocora', 'Llano Grande', 'Palestina', 'Canaán',
    ]),
    '63272': ('filandia', 'needs', [
        'Centro', 'La India', 'Cruces', 'El Placer', 'La Julia', 'Bambuco Bajo',
    ]),
    '76147': ('cartago', 'needs', [
        'Centro', 'Santa Ana', 'El Prado', 'La Platanera', 'Zaragoza', 'San Jerónimo',
    ]),
    '76834': ('tulua', 'needs', [
        'Centro', 'Victoria', 'Alvernia', 'Sajonia', 'La Trinidad', 'Farfán',
    ]),
    '76111': ('buga', 'needs', [
        'Centro', 'La Merced', 'José María Cabal', 'El Albergue', 'Santa Bárbara', 'Fuenmayor',
    ]),
    '76520': ('palmira', 'needs', [
        'Centro', 'Zamorano', 'La Emilia', 'Las Delicias', 'Santa Rita', 'El Recreo',
    ]),
    '76892': ('yumbo', 'needs', [
        'Centro', 'Las Américas', 'Uribe', 'Bellavista', 'Puerto Isaacs', 'Guacandá',
    ]),
    # --- SUPPORT: rest of the country, 8 points each, offers help
    '11001': ('bogota', 'offers', [
        'Chapinero', 'Suba', 'Kennedy', 'Usaquén', 'Teusaquillo', 'Engativá',
        'Fontibón', 'Bosa',
    ]),
    '05001': ('medellin', 'offers', [
        'El Poblado', 'Laureles', 'Belén', 'Robledo', 'Manrique', 'Castilla',
        'Aranjuez', 'Buenos Aires',
    ]),
    '68001': ('bucaramanga', 'offers', [
        'Cabecera', 'San Francisco', 'Girardot', 'La Concordia', 'Provenza',
        'Álvarez', 'Sotomayor', 'Mutis',
    ]),
    '08001': ('barranquilla', 'offers', [
        'El Prado', 'Alto Prado', 'Riomar', 'Las Flores', 'Rebolo', 'La Concepción',
        'Boston', 'Simón Bolívar',
    ]),
    '73001': ('ibague', 'offers', [
        'La Pola', 'Belén', 'Cádiz', 'El Salado', 'Picaleña', 'Ambalá',
        'El Jordán', 'Interlaken',
    ]),
    '41001': ('neiva', 'offers', [
        'Centro', 'Las Granjas', 'Cándido', 'El Altico', 'Ipanema', 'Los Mártires',
        'Timanco', 'Buganviles',
    ]),
}
...
BULK_RESOURCES = [
    ('agua', 400), ('alimentos', 150), ('medicamentos', 40), ('carpas', 15),
    ('colchonetas', 60), ('voluntarios', 10), ('alimentos_mascotas', 25),
    ('refugio', 20),
]
...
def _is_test_database(schema_editor) -> bool:
    return schema_editor.connection.settings_dict.get('NAME', '').startswith('test_')


def seed(apps, schema_editor):
    if _is_test_database(schema_editor):
        return  # tests build their own fixtures; seeding here collides with factories

    AdminUnit = apps.get_model('radar', 'AdminUnit')
    ResourceType = apps.get_model('radar', 'ResourceType')
    Event = apps.get_model('radar', 'Event')
    HarvestJob = apps.get_model('radar', 'HarvestJob')
    Observation = apps.get_model('radar', 'Observation')
    Extraction = apps.get_model('radar', 'Extraction')
    Location = apps.get_model('radar', 'Location')
    Actor = apps.get_model('radar', 'Actor')
    ContactPoint = apps.get_model('radar', 'ContactPoint')
    Requirement = apps.get_model('radar', 'Requirement')
    FrontierNode = apps.get_model('radar', 'FrontierNode')
    ...
    def make_requirement(actor, direction, resource_key, quantity, covered, urgency,
                         location, destination, text, observation, confidence=0.85):
        ...
        requirement = Requirement.objects.create(
            ...
            urgency=urgency,
            status='partial' if 0 < covered < quantity else (
                'covered' if covered >= quantity else 'open'
            ),
            confidence=confidence,
            last_seen_at=now,
        )
    ...
    for i, row in enumerate(REQUIREMENTS):
        (slug, actor_slug, direction, resource_key, qty, covered, unit, urgency,
         point_key, admin_code, dest_point_key, dest_admin_code, text) = row
        observation = make_observation(f'demo-{slug}', text, actors[actor_slug].canonical_name, 30 - i)
        make_requirement(
            actors[actor_slug], direction, resource_key, qty, covered, urgency,
    ...
    for admin_code, (center_key, main_direction, barrios) in ZONES.items():
        ...
        for i, barrio in enumerate(barrios):
            ...
            direction = main_direction if i % 4 else (
                'offers' if main_direction == 'needs' else 'needs'
            )
            ...
            make_requirement(
                actor, direction, resource_key, quantity, 0,
                BULK_URGENCIES[i % len(BULK_URGENCIES)],
                actor.location, None, text, observation,
...
def unseed(apps, schema_editor):
    ...
    Location.objects.filter(
        source='manual', admin_unit__country_code='CO',
...
class Migration(migrations.Migration):
    dependencies = [
        ...
        ('radar', '0003_postgres_extensions'),
    ]
